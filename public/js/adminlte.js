/*!
 * AdminLTE vundefined (undefined)
 * Copyright 2014-2021 undefined
...
(function (global, factory) {
  ...
  const JQUERY_NO_CONFLICT = window.$.fn[NAME];
  ...
  class ControlSidebar {
    ...
    _fixScrollHeight() {
      ...
      const navbarFixed = ($body.hasClass(CLASS_NAME_NAVBAR_FIXED) || $body.hasClass(CLASS_NAME_NAVBAR_SM_FIXED) || $body.hasClass(CLASS_NAME_NAVBAR_MD_FIXED) || $body.hasClass(CLASS_NAME_NAVBAR_LG_FIXED) || $body.hasClass(CLASS_NAME_NAVBAR_XL_FIXED)) && $__default['default'](SELECTOR_HEADER).css('position') === 'fixed';
      const footerFixed = ($body.hasClass(CLASS_NAME_FOOTER_FIXED) || $body.hasClass(CLASS_NAME_FOOTER_SM_FIXED) || $body.hasClass(CLASS_NAME_FOOTER_MD_FIXED) || $body.hasClass(CLASS_NAME_FOOTER_LG_FIXED) || $body.hasClass(CLASS_NAME_FOOTER_XL_FIXED)) && $__default['default'](SELECTOR_FOOTER).css('position') === 'fixed';
      const $controlSidebar = $__default['default'](SELECTOR_CONTROL_SIDEBAR);
      const $controlsidebarContent = $__default['default'](`${SELECTOR_CONTROL_SIDEBAR}, ${SELECTOR_CONTROL_SIDEBAR} ${SELECTOR_CONTROL_SIDEBAR_CONTENT}`);
    ...
    _fixHeight() {
      ...
      const $controlSidebar = $__default['default'](`${SELECTOR_CONTROL_SIDEBAR} ${SELECTOR_CONTROL_SIDEBAR_CONTENT}`);
    ...
    search() {
      ...
      if (endResults.length === 0) {
        ...
        endResults.each((i, result) => {
          $__default['default'](SELECTOR_SEARCH_RESULTS_GROUP).append(this._renderItem(result.name, result.link, result.path));
        });
      }
  ...
  const NAME$b = 'Toasts';
  const DATA_KEY$b = 'lte.toasts';
  const EVENT_KEY$6 = `.${DATA_KEY$b}`;
  ...
  class Toasts {
    ...
    static _jQueryInterface(option, config) {
      return this.each(function () {
        const _options = $__default['default'].extend({}, Default$9, config);
  ...
  const NAME$c = 'TodoList';
  const DATA_KEY$c = 'lte.todolist';
  ...
  class TodoList {
    ...
    static _jQueryInterface(config) {
      return this.each(function () {
        ...
        const _options = $__default['default'].extend({}, Default$a, typeof config === 'object' ? config : data);
  ...
  const NAME$d = 'Treeview';
  const DATA_KEY$d = 'lte.treeview';
  const EVENT_KEY$7 = `.${DATA_KEY$d}`;
  ...
  const CLASS_NAME_OPEN$2 = 'menu-open';
  ...
  class Treeview {
    ...
    toggle(event) {
      ...
      const isOpen = parentLi.hasClass(CLASS_NAME_OPEN$2);
